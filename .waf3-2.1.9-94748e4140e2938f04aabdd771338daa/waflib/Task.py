...
class Task(metaclass=store_task_type):
	...
	def exec_command(self, cmd, **kw):
		...
		if self.generator.bld.progress_bar == 1:
			kw['output_prefix'] = self.display_output_prefix()
		return self.generator.bld.exec_command(cmd, **kw)

	def display_output_prefix(self):
		"""
		Returns task description string for prefixing command output
		"""
		col1 = Logs.colors(self.color)
		col2 = Logs.colors.NORMAL
		master = self.generator.bld.producer
		cur = master.processed - master.ready.qsize()
		return self.display_task_line(cur, master.total, col1, col2) or ''
	...
	def display(self):
		...
		return self.display_task_line(cur(), master.total, col1, col2)

	def display_task_line(self, cur, total, col1, col2):
		"""
		Returns task description line for the console.
