...
import su.xash.engine.util.GameInstaller
	...
	private var _binding: FragmentLibraryBinding? = null
		...
		private val libraryViewModel: LibraryViewModel by activityViewModels()
				...
				if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
					if (!Environment.isExternalStorageManager()) {
						...
						null
							...
							setNeutralButton(
								R.string.done_check_permissions
							) { _, _ ->
								checkStoragePermissions()
							}
			...
			override fun onViewCreated(
				...
				libraryViewModel.isReloading.observe(
					viewLifecycleOwner
				) { isReloading ->
					binding.swipeRefresh.isRefreshing = isReloading
				}

				libraryViewModel.installedGames.observe(
					viewLifecycleOwner
				) { games ->
					(binding.gamesList.adapter as GameAdapter).submitList(games)
				}
			...
			override fun onMenuItemSelected(
				...
				return when (menuItem.itemId) {
					R.id.action_install_game -> {
						GameInstallerDialog.show(
							requireContext()
						) { selectedGame ->
					...
					R.id.action_settings -> {
						findNavController().navigate(
							R.id.action_libraryFragment_to_appSettingsFragment
						)
